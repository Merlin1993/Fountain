...
func TestSign(t *testing.T) {
	for _, test := range testCases { //多个测试用例
		if test.d == "" {
			continue
		}

		// given
		d := decodePrivateKey(test.d, t) //从测试用例中获取私钥
		m := decodeMessage(test.m, t)    //从测试用例中获取签名的消息

		// when
		result, err := Sign(d, m) //生成签名
		if err != nil {
			t.Fatalf("Unexpected error from Sign(%s, %s): %v", test.d, test.m, err)
		}

		observed := hex.EncodeToString(result[:]) //生成的签名转为string形式，
		expected := strings.ToLower(test.sig)     //从测试用例中获取签名

		// then 生成的签名 与 从测试用例中获取的签名，进行比对，
...
func TestVerify(t *testing.T) {
	for _, test := range testCases {
		// given 从测试用例中拿到签名的公钥pk、对象m、签名sig
